...
bool ConnectionManager::checkIpFlood(const string& aServer, uint16_t aPort, const string& userInfo) {
	...
	if(aPort == 80 || aPort == 2501) {
		LogManager::getInstance()->message("Someone (" + userInfo + ") is trying to use your client to spam " + aServer + ":" + Util::toString(aPort) + ", please urge hub owner to fix this", LogManager::LOG_WARNING);
		return true;
	}	
...
/**
 * @file
 * $Id: ConnectionManager.cpp 562 2011-01-09 12:54:12Z bigmuscle $
 */
