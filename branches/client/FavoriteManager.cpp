...
void FavoriteManager::userUpdated(const OnlineUser& info) {
	...
	i = ignoredUsers.find(info.getUser()->getCID());
	if(i != ignoredUsers.end()) {
		FavoriteUser& iu = i->second;
		iu.update(info);
		shouldSave = true;
	}
...
time_t FavoriteManager::getLastSeen(const UserPtr& aUser) const { 
	...
	// ignored and favorite users must have same "last seen" for same CID!
	FavoriteMap::const_iterator i = users.find(aUser->getCID());
	if(i == users.end())
	{
		i = ignoredUsers.find(aUser->getCID());
		if(i == ignoredUsers.end())
			return 0;
	}
...
void FavoriteManager::on(UserDisconnected, const UserPtr& user) throw() {
	{
		...
		if(i == users.end()) {
			i = ignoredUsers.find(user->getCID());
			if(i == ignoredUsers.end())
				return;
		}
...
void FavoriteManager::on(UserConnected, const UserPtr& user) throw() {
	{
		Lock l(cs);
		if(	users.find(user->getCID()) == users.end() && 
			ignoredUsers.find(user->getCID()) == ignoredUsers.end()) 
		{
			return;
		}
	}
...
/**
 * @file
 * $Id: FavoriteManager.cpp 559 2010-12-30 23:57:56Z bigmuscle $
 */
