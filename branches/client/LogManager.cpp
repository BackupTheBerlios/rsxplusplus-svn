...
void LogManager::message(const string& msg, Severity severity) {
	...
	{
		Lock l(cs);
		// Keep the last 10 messages (completely arbitrary number...)
		// we just need it to display logs before GUI opens
		while(lastLogs.size() > 10)
			lastLogs.pop_front();
		lastLogs.push_back(make_pair(t, make_pair(msg, severity)));
	}
...
/**
 * @file
 * $Id: LogManager.cpp 562 2011-01-09 12:54:12Z bigmuscle $
 */
