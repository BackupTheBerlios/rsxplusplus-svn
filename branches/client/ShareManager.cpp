		...
		if(BOOLSETTING(REMOVE_FORBIDDEN)) {
			...
			if ((stricmp(fileExt.c_str(), ".tdc") == 0) ||
				...
				(nameLen >= 28 && name.find("download") == 0 && name.rfind(".dat") == nameLen - 4 && Util::toInt64(name.substr(8, 16)))) {		//kazaa temps
					LogManager::getInstance()->message("Forbidden file will not be shared: " + name + " (" + STRING(SIZE) + ": " + Util::toString(File::getSize(name)) + " " + STRING(B) + ") (" + STRING(DIRECTORY) + ": \"" + aName + "\")", LogManager::LOG_WARNING);
					continue;
			}
		}
	...
	{		
		...
		if(thoroughCheck)
		{
			if(path.size() > (*j).size())
			{
				...
				if((stricmp(temp, *j) == 0) && (path[(*j).size() - 1] == '\\'))
					return false;
			}
		}
	}
...
/**
 * @file
 * $Id: ShareManager.cpp 562 2011-01-09 12:54:12Z bigmuscle $
 */
