...
void Socket::writeTo(const string& aAddr, uint16_t aPort, const void* aBuffer, int aLen, bool proxy) throw(SocketException) {
	...
	if(aPort == 80 || aPort == 2501) {
		LogManager::getInstance()->message("Someone is trying to use your client to spam " + aAddr + ", please urge hub owner to fix this", LogManager::LOG_WARNING);
		return;
	}		
...
/**
 * @file
 * $Id: Socket.cpp 562 2011-01-09 12:54:12Z bigmuscle $
 */
