...
function logfunctions.loadSettings()
	local o = io.open( DC():GetAppPath() .. "/scripts/log_settings.txt", "r" )
	if o then
		o:close()
		dofile( DC():GetAppPath() .. "/scripts/log_settings.txt" )
	end
	logfunctions.updateConfig()
end

function logfunctions.saveSettings()
	pickle.store(DC():GetAppPath() .. "/scripts/log_settings.txt", { logsettings = logsettings })
end
