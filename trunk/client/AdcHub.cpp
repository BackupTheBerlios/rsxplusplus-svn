...
#define ADDPARAM(var, content) \
	...
	ADDPARAM("SS", getHideShare() ? "0" : ShareManager::getInstance()->getShareSizeString()); //rsxpp
	ADDPARAM("SF", getHideShare() ? "0" : Util::toString(ShareManager::getInstance()->getSharedFiles())); //rsxpp
	ADDPARAM("EM", /*SETTING(EMAIL)*/getCurrentEmail());
	ADDPARAM("HN", Util::toString(counts.normal));
	ADDPARAM("HR", Util::toString(counts.registered));
	ADDPARAM("HO", Util::toString(counts.op));
	ADDPARAM("VE", getStealth() ? ("++ " DCVERSIONSTRING) : ("RSX++ " VERSIONSTRING));
...
/**
 * @file
 * $Id: AdcHub.cpp 334 2007-11-04 13:04:34Z bigmuscle $
 */
