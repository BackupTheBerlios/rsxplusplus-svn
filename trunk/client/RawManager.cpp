...
Raw* RawManager::addRaw(Action* a, Raw& r) {
	...
	for(Action::RawsList::const_iterator j = a->raw.begin(); j != a->raw.end(); ++j) {
		if(stricmp(j->getName(), r.getName()) == 0)
...
void RawManager::editRaw(const Action* a, Raw* old, Raw& _new) throw(Exception) {
	if(_new.getName().empty())
		throw Exception(STRING(NO_NAME_SPECIFIED));
	if(stricmp(old->getName(), _new.getName()) != 0) {
		Lock l(cs);
		for(Action::RawsList::const_iterator j = a->raw.begin(); j != a->raw.end(); ++j) {
			if(stricmp(j->getName(), _new.getName()) == 0)
				throw Exception(STRING(RAW_EXISTS));
		}
	}
	*old = _new;
...
void RawManager::calcADLAction(int aPoints, int& a, bool& d) {
	...
	if(a == 0) {
		a = RSXPP_SETTING(ADLSEARCH_DEFAULT_ACTION);
	}

	int min_p = RSXPP_SETTING(MIN_POINTS_TO_DISPLAY_CHEAT);
	if(min_p > 0) {
		d = (aPoints >= min_p) && RSXPP_BOOLSETTING(SHOW_ADLSEARCH_DEFAULT_ACTION);
	} else {
		d = RSXPP_BOOLSETTING(SHOW_ADLSEARCH_DEFAULT_ACTION);
	}
}
