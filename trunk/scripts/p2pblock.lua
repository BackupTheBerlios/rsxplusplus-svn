...
dcpp:setListener( "clientIn", "p2pblock",
	function( userp, line )
		...
		if string.sub( line, 1, 8 ) == "$MyNick " then
			...
			if p2pblock.settings.nicks[nick] then
				DC():PrintDebug( "p2pblock: match" )
...
function p2pblock.add( hub, nick )
	hub:injectChat( "*** p2pblock.lua: adding nick \"" .. nick .. "\"" )
	p2pblock.settings.nicks[nick] = 1
	p2pblock.save()
end

function p2pblock.del( hub, nick )
	hub:injectChat( "*** p2pblock.lua: removing nick \"" .. nick .. "\"" )
	p2pblock.settings.nicks[nick] = nil
	p2pblock.save()
end
