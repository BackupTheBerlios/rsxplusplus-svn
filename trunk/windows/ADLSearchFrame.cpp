...
static ResourceManager::Strings columnNames[] = { 
	...
	//RSX++
	ResourceManager::ADL_POINTS,
	ResourceManager::ACTION,
	ResourceManager::ADL_OVERRIDE_POINTS_COLUMN
	//END
};
...
LRESULT ADLSearchFrame::onCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	ctrlList.SetExtendedListViewStyle(LVS_EX_LABELTIP | LVS_EX_HEADERDRAGDROP | LVS_EX_CHECKBOXES | LVS_EX_FULLROWSELECT | LVS_EX_INFOTIP);
...
void ADLSearchFrame::UpdateSearch(int index, BOOL doDelete)
{
	...
	//RSX++
	line.push_back(Util::toStringW(search.adlsPoints));
	line.push_back(RawManager::getInstance()->getNameActionId(search.adlsRaw));
	line.push_back(WinUtil::toYesNo(search.overRidePoints));
	//END
...
//RSX++
LRESULT ADLSearchFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	case CDDS_ITEMPREPAINT:
		{
			...
			try	{
				ADLSearchManager::SearchCollection& collection = ADLSearchManager::getInstance()->collection;
				ADLSearch search = collection[cd->nmcd.dwItemSpec];
				if (search.isForbidden && search.isActive) {
