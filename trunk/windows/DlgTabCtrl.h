...
public:
	...
	BOOL AddTab(LPWSTR tbName, HWND wnd, int imgIndex = -1, bool xpStyle = false) {
	   	TCITEM tci = { 0 };
	...
	int CalcTabHeight() {
		int nCount = m_ctrlViews.GetItemCount();
		TCITEM tcix = { 0 };
		tcix.mask = TCIF_TEXT;
		tcix.pszText = _T("NS");
		int nIndex = InsertItem(nCount, &tcix, 0);
		RECT rect = { 0, 0, 1000, 1000 };
		AdjustRect(FALSE, &rect);
		RECT rcWnd = { 0, 0, 1000, rect.top };
		::AdjustWindowRectEx(&rcWnd, /*m_ctrlViews.GetStyle()*/0, FALSE, /*m_tab.GetExStyle()*/0);
	...
	void UpdateLayout() {
		...
		//GetClientRect(&rect);
		//CalcTabHeight();
		//if(m_ctrlViews.IsWindow() /*&& ((m_tab.GetStyle() & WS_VISIBLE) != 0)*/)
		//	m_ctrlViews.SetWindowPos(NULL, 0, 0, rect.right - rect.left, 500, SWP_NOZORDER);
