...
PropPage::TextItem OperaColorsPage::texts[] = {
	...
	{ IDC_INDICATE_SPEEDS, ResourceManager::STEALTHY_INDICATE_SPEEDS },
...
LRESULT OperaColorsPage::onInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	ctrlMode.Attach(GetDlgItem(IDC_PROGRESS_MODE));
	ctrlMode.AddString(_T("Normal Style"));
	ctrlMode.AddString(_T("oDC Style"));
	ctrlMode.AddString(_T("Stealthy Style"));
	ctrlMode.SetCurSel(SETTING(PROGRESSBAR_MODE));
...
void OperaColorsPage::write()
{
	...
	SettingsManager::getInstance()->set(SettingsManager::PROGRESSBAR_MODE, ctrlMode.GetCurSel());
...
//	if (wParam == IDC_SETTINGS_ODC_MENUBAR_COLOR) {
		...
		if (dis->CtlType == ODT_STATIC) {
			...
			if (dis->CtlID == IDC_SETTINGS_ODC_MENUBAR_COLOR) {
				...
				int textcolor;
				if(ctrlMode.GetCurSel() == 2) { //stealthly
					// if one finds a better/cleaner/nicer way to do this, please tell me...
					CDC cdc;
					...
					cdc.Detach();
				} else if(ctrlMode.GetCurSel() == 1) { //odc
					COLORREF a, b;
...
LRESULT OperaColorsPage::onClickedProgress(WORD /* wNotifyCode */, WORD wID, HWND /* hWndCtl */, BOOL& /* bHandled */) {
	odcStyle = ctrlMode.GetCurSel() == 1;
	stealthyStyle = ctrlMode.GetCurSel() == 2;
	updateProgress();
...
void OperaColorsPage::updateProgress() {
	stealthyStyle =	(ctrlMode.GetCurSel() == 2);
	::EnableWindow(GetDlgItem(IDC_ODC_STYLE), !stealthyStyle);
	::EnableWindow(GetDlgItem(IDC_INDICATE_SPEEDS), stealthyStyle);

	odcStyle = (ctrlMode.GetCurSel() == 1);
	::EnableWindow(GetDlgItem(IDC_PROGRESS_BUMPED), (ctrlMode.GetCurSel() == 1));

	bool state = (IsDlgButtonChecked(IDC_PROGRESS_OVERRIDE) != 0);
	::EnableWindow(GetDlgItem(IDC_PROGRESS_OVERRIDE2), state && !stealthyStyle);
	::EnableWindow(GetDlgItem(IDC_SETTINGS_DOWNLOAD_BAR_COLOR), state);
	::EnableWindow(GetDlgItem(IDC_SETTINGS_UPLOAD_BAR_COLOR), state);

	::EnableWindow(GetDlgItem(IDC_FLAT), ctrlMode.GetCurSel() == 0);
	::EnableWindow(GetDlgItem(IDC_FLAT_SPIN), ctrlMode.GetCurSel() == 0);

	state = (IsDlgButtonChecked(IDC_INDICATE_SPEEDS) != 0);
	::EnableWindow(GetDlgItem(IDC_TOP_SPEED), stealthyStyle && state);
	::EnableWindow(GetDlgItem(IDC_SPEED_STR), stealthyStyle && state);
	::EnableWindow(GetDlgItem(IDC_KBPS), stealthyStyle && state);
	::EnableWindow(GetDlgItem(IDC_TOP_UP_SPEED), stealthyStyle && state);
	::EnableWindow(GetDlgItem(IDC_SPEED_UP_STR), stealthyStyle && state);
	::EnableWindow(GetDlgItem(IDC_KBPS2), stealthyStyle && state);

	state = ((IsDlgButtonChecked(IDC_PROGRESS_OVERRIDE) != 0) && (IsDlgButtonChecked(IDC_PROGRESS_OVERRIDE2) != 0));
	::EnableWindow(GetDlgItem(IDC_PROGRESS_TEXT_COLOR_DOWN), state && !stealthyStyle);
	::EnableWindow(GetDlgItem(IDC_PROGRESS_TEXT_COLOR_UP), state && !stealthyStyle);
...
LRESULT OperaColorsPage::onSelChange(WORD /* wNotifyCode */, WORD /*wID*/, HWND /* hWndCtl */, BOOL& /* bHandled */) {
	updateProgress();
	return 0;
}
