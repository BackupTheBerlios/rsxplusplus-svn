...
HTREEITEM QueueFrame::addDirectory(const string& dir, bool isFileList /* = false */, HTREEITEM startAt /* = NULL */, bool ts /*= false*/, bool fl /*= false*/) {
	TVINSERTSTRUCT tvi;
	...
	if(ts) {
		...
		tvi.hParent = NULL;
		tvi.item.pszText = _T("Client checks");
		tvi.item.lParam = (LPARAM) new string(dir);
		testsurRoot = ctrlDirs.InsertItem(&tvi);
		return testsurRoot;
	} else if(fl) {
		dcassert(flcRoot == NULL);
		tvi.hParent = NULL;
		tvi.item.pszText = _T("FileList checks");
		tvi.item.lParam = (LPARAM) new string(dir);
		flcRoot = ctrlDirs.InsertItem(&tvi);
		return flcRoot;
	} else
	//END
	if(isFileList) {
...
void QueueFrame::removeDirectory(const string& dir, bool isFileList /* = false */, bool ts /*= false*/, bool fl /*= false*/) {
	...
	if(ts) {
		dcassert(testsurRoot != NULL);
		delete (string*)ctrlDirs.GetItemData(testsurRoot);
		ctrlDirs.DeleteItem(testsurRoot);
		testsurRoot = NULL;
		return;
	} else if(fl) {
		dcassert(flcRoot != NULL);
		delete (string*)ctrlDirs.GetItemData(flcRoot);
		ctrlDirs.DeleteItem(flcRoot);
		flcRoot = NULL;
		return;
	} else
	//END
	...
	for(TaskQueue::Iter ti = t.begin(); ti != t.end(); ++ti) {
		if(ti->first == ADD_ITEM) {
			...
			if(directories.count(ii->getPath()) == 0) {
				removeDirectory(ii->getPath(), ii->isSet(QueueItem::FLAG_USER_LIST), ii->isSet(QueueItem::FLAG_TESTSUR), ii->isSet(QueueItem::FLAG_CHECK_FILE_LIST));
