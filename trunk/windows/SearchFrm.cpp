...
void SearchFrame::on(SearchManagerListener::SR, SearchResult* aResult) throw() {
	...
	//RSX++
	if(SETTING(PG_ENABLE) && SETTING(PG_SEARCH)) {
		if(PGManager::getInstance()->notAbused()) {
			...
			string company = PGManager::getInstance()->getIPBlock(aResult->getIP());
			if (!company.empty()) {
				droppedResults++;
...
/**
 * @file
 * $Id: SearchFrm.cpp 334 2007-11-04 13:04:34Z bigmuscle $
 */
